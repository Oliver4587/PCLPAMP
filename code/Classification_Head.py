...
class Encoder(nn.Module):
    def __init__(self, in_dim=1024, hidden_dim=2048, out_dim=1024):
...
class Conv2D_Layer1(nn.Module):
    def __init__(self):
        super(Conv2D_Layer1, self).__init__()
        self.convnet = nn.Sequential()
        #N 1@200*1024 -> 6@200*1024
        self.convnet.add_module('Conv_1',nn.Conv2d(in_channels=1,out_channels=6, kernel_size=11, stride = 1, padding=5 ))
        self.convnet.add_module('activation_1',nn.ReLU())
        # 320@200*1024 -> 6@25*128
        self.convnet.add_module('Max Pooling1', nn.MaxPool2d(kernel_size=2, stride=8))

        #6@25*128 -> 16@21*124
        self.convnet.add_module('Conv_2',nn.Conv2d(in_channels=6,out_channels=16, kernel_size=5, stride = 1))
        self.convnet.add_module('activation_2',nn.ReLU())
        #16@21*124 ->16@10*62
        self.convnet.add_module('Max Pooling2', nn.MaxPool2d(kernel_size=2))
        self.convnet.add_module("BatchNormalization",nn.BatchNorm2d(num_features=16, momentum=0.99))
        
        self.convnet.add_module("Flatten",nn.Flatten()) 
        self.convnet.add_module ("Fc",nn.Linear(in_features=16*10*62, out_features=2))
